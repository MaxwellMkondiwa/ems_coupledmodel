...
undirected-link-breed [pairlinks pairlink]
...
to init-agents
  ...
  set PastSysEffList (list 0)
  set CurrentAdopter 1
  build-wells
  ]
...
to go
  ...
  ask systems
  [set SystemCumEinW sum [cumEin] of WellsOwned with [IsCold = 0]
   set SystemCumEinC abs sum [cumEin] of WellsOwned with [IsCold = 1]
   let SystemCumElossW sum [cumEloss] of WellsOwned with [IsCold = 0]
   let SystemCumElossC abs sum [cumEloss] of WellsOwned with [IsCold = 1]
   set SystemCumEoutW SystemCumEinW - SystemCumElossW
   set SystemCumEoutC SystemCumEinC - SystemCumElossC

  if SystemCumEinW > 0 and SystemCumEinC > 0 [
    let SystemEfftot (SystemCumEoutW + SystemCumEoutC) / (SystemCumEinW + SystemCumEinC)
    set MeanSysEff min list (max list SystemEfftot 0) 1
    ifelse length PastSysEffList < 24
     [set PastSysEffList lput MeanSysEff PastSysEffList]
     [set PastSysEffList lput MeanSysEff but-first PastSysEffList]]

  ]

  ask systems [
    if random 100 < BuildThreshold and mean PastSysEffList > 0.8 and CurrentAdopter = 1
    [build-wells]
  ]

 ask systems
 [if ticks > 48 and CurrentAdopter = 1 and mean PastSysEffList < 0.7
   [ask WellsOwned [set IsActive 0]
   set CurrentAdopter 0]]
 if ticks > 24
 [let MeanEffVal (sum [SystemCumEoutW] of systems + sum [SystemCumEoutC] of systems) / (sum [SystemCumEinW] of systems + sum [SystemCumEinC] of systems)
 set MeanEff min list (max list MeanEffVal 0) 1]
...
to build-wells
  ...
  let AvailablePatches patches with [not any? wells in-radius MinRadius]
  if any? AvailablePatches
  [hatch-wells 1
  [set Parent myself
   set IsActive 1
   set IsCold 0
   set T_inj 15
   set z0 ztop
   set FilterLength Laquifer
   set color red - 2
   move-to one-of AvailablePatches
   set AvailablePatches patches with [not any? wells in-radius MinRadius]
   ifelse any? AvailablePatches
   [hatch-wells 1
  [set IsCold 1
   set T_inj 5
   set color blue - 2
   create-pairlink-with myself
   move-to one-of AvailablePatches]


  ask max-n-of 2 wells [who] [
      set PairedWell min-one-of pairlink-neighbors [who]]]
  [die]]]

  set WellsOwned wells with [Parent = myself]
...
to calc-performance
  ...
  ifelse Q < 0
   [set Eloss Q * (T_modflow - T_inj) * 4186 * 1000 / 1e9 * 30 ;GJ
    set cumEloss cumEloss + Eloss
    set Ein 0]
   [set Ein Q * (T_inj - [T_modflow] of PairedWell) * 4186 * 1000 / 1e9 * 30
    set cumEin cumEin + Ein
    set Eloss 0]
...
"" ""
PENS
"pen-1" 1.0 0 -7500403 true "" "plot count wells"
"pen-2" 1.0 0 -2674135 true "" "plot count wells with [IsActive = 1]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot sum [Q] of wells with [IsCold = 1]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot SubsurfaceUse"
